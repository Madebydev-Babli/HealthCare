...
import { CalendarDays, Clock3, Filter, Search, Stethoscope } from "lucide-react";
...
export default function AppointmentsPage() {
  ...
  const filteredAppointments = useMemo(() => {
    ...
    const normalized = list.filter((appointment) => {
      ...
      const matchesSearch = !search || doctorName.includes(search.toLowerCase());
    ...
    return [...normalized].sort((a, b) => parseDateTime(a.date, a.time) - parseDateTime(b.date, b.time));
      ...
      <div className="mx-auto max-w-6xl">
        <div className="rounded-[30px] border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-cyan-600">Appointments</p>
              <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">My Appointments</h1>
            </div>
          ...
          <div className="mt-6 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div className="relative w-full md:max-w-sm">
              ...
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-10 pr-4 text-sm outline-none transition focus:border-cyan-500 focus:bg-white"
              />
            </div>
        ...
        {!loading && unauth && (
          <div className="mt-8 rounded-[28px] border border-slate-200 bg-white p-8 text-center shadow-sm">
            <p className="text-xl font-semibold text-slate-900">Please sign in to view your appointments.</p>
            <Link href="/auth/login" className="mt-5 inline-flex items-center rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-cyan-700">
              Sign In
            </Link>
          </div>
        )}
        ...
        {!loading && !unauth && filteredAppointments.length === 0 && (
          <div className="mt-8 rounded-[28px] border border-dashed border-slate-300 bg-white p-10 text-center shadow-sm">
            ...
            <h2 className="mt-5 text-2xl font-semibold text-slate-900">No appointments found</h2>
            <p className="mt-2 text-slate-600">Your selected filter is empty right now.</p>
            <Link href="/book-appointment" className="mt-6 inline-flex items-center rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-cyan-700">
              Find a Doctor
            </Link>
          </div>
        )}
        ...
        {!loading && !unauth && filteredAppointments.length > 0 && (
          <div className="mt-8 space-y-4">
            {filteredAppointments.map((appointment) => (
              <div key={String(appointment._id)} className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm transition hover:shadow-md sm:p-6">
                <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <p className="text-xl font-bold text-slate-900">{appointment.doctorName || "Doctor"}</p>
                      <p className="mt-1 text-sm text-slate-500">{appointment.doctorId?.fieldOfMedical || appointment.specialization || "Healthcare consultation"}</p>
                      <div className="mt-3 flex flex-wrap items-center gap-3 text-sm text-slate-500">
                        <span className="inline-flex items-center gap-1.5"><CalendarDays size={15} /> {appointment.date}</span>
                        <span className="inline-flex items-center gap-1.5"><Clock3 size={15} /> {appointment.time}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col items-start gap-3 sm:flex-row sm:items-center">
                    <div className="rounded-2xl bg-slate-50 px-4 py-2 text-left text-sm">
                      ...
                      <p className="font-semibold text-slate-900">₹{appointment.fee || 0}</p>
                    </div>
